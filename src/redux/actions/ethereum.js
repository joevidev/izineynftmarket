...
import IzineyNFT from '../../abis/IzineyNFT.json'
...
export const loadBlockchainData = () => async dispatch => {
    if(window.web3){
        ...
        const izineyNFTData = IzineyNFT.networks[networkId]
        if(izineyNFTData){
            const abi = IzineyNFT.abi
            const address = izineyNFTData.address
            const contract = new web3.eth.Contract(abi, address)
            dispatch({
                type:LOAD_IZINEYNFT_CONTRACT_SUCCESS,
                payload:contract
            })
            ...
            if(totalSupply > 0){
                ...
                for(let i = 1; i <= totalSupply; i++) {
                    const IzineyNFT = await contract.methods.IzineyNFTS(i - 1).call()
                    nfts.push(IzineyNFT)
                    dispatch({
                        type: LOAD_IZINEYNFT_NFTS_SUCCESS,
                        payload: nfts
                    })
                }
        ...
        dispatch({
            type:LOAD_IZINEYNFT_CONTRACT_FAIL,
        })
    }
}
